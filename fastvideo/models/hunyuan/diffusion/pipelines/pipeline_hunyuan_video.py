...
class HunyuanVideoPipeline(DiffusionPipeline):
    ...
    def encode_prompt(
        ...
        if negative_prompt_embeds is None and do_classifier_free_guidance:
            # textual inversion: process multi-vector tokens if necessary
            if isinstance(self, TextualInversionLoaderMixin):
                negative_prompt = self.maybe_convert_prompt(negative_prompt, text_encoder.tokenizer)
            ...
            if clip_skip is None:
                ...
                # obtaining the final prompt representations passes through the LayerNorm
                # layer.
                negative_prompt_embeds = text_encoder.model.text_model.final_layer_norm(negative_prompt_embeds)
            
            negative_attention_mask = negative_prompt_outputs.attention_mask
            if negative_attention_mask is not None:
                negative_attention_mask = negative_attention_mask.to(device)
                negative_bs_embed, negative_seq_len = negative_attention_mask.shape
                negative_attention_mask = negative_attention_mask.repeat(1, num_videos_per_prompt)
                negative_attention_mask = negative_attention_mask.view(negative_bs_embed * num_videos_per_prompt, negative_seq_len)
        ...
        if prompt_embeds.ndim == 2:
            ...
            if do_classifier_free_guidance:
                negative_bs_embed, _ = negative_prompt_embeds.shape
                # duplicate text embeddings for each generation per prompt, using mps friendly method
                negative_prompt_embeds = negative_prompt_embeds.repeat(1, num_videos_per_prompt)
                negative_prompt_embeds = negative_prompt_embeds.view(negative_bs_embed * num_videos_per_prompt, -1)
        else:
            ...
            if do_classifier_free_guidance:
                negative_bs_embed, negative_seq_len, _ = negative_prompt_embeds.shape
                # duplicate text embeddings for each generation per prompt, using mps friendly method
                negative_prompt_embeds = negative_prompt_embeds.repeat(1, num_videos_per_prompt, 1)
                negative_prompt_embeds = negative_prompt_embeds.view(negative_bs_embed * num_videos_per_prompt, negative_seq_len, -1)
    ...
    def __call__(
        ...
        videos_org: Optional[Union[torch.Tensor, list]],
        height: int,
        width: int,
        video_length: int,
        data_type: str = "video",
        ...
        **kwargs,
        ...
        with torch.autocast(device_type="cuda", dtype=vae_dtype, enabled=vae_autocast_enabled):
            video_latents = []
            for video_org in videos_org:
                assert isinstance(video_org, torch.Tensor)
                video_latents.append(self.vae.encode(video_org).latent_dist.mode())
                video_latents[-1].mul_(self.vae.config.scaling_factor)

        # 5. Prepare latent variables
        num_channels_latents = self.transformer.config.in_channels
        latents_list = []
        for i in range(output_num): # NOTE: flexible output num
            latents_list.append(self.prepare_latents(
                batch_size,
                num_channels_latents,
                height,
                width,
                video_length, # NOTE: fixed to 1
                video_latents[0].dtype,
                device,
                generator,
                latents,
            ))
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = []
                latent_model_input_org = []
                for latent in latents:
                    latent_in = (torch.cat([latent] * 2) if self.do_classifier_free_guidance else latent)
                    latent_in = self.scheduler.scale_model_input(latent_in, t)
                    latent_model_input.append(latent_in)
                for latent_org in video_latents:   
                    latent_org_in = (torch.cat([latent_org] * 2) if self.do_classifier_free_guidance else latent_org)
                    latent_org_in = self.scheduler.scale_model_input(latent_org_in, t)
                    latent_model_input_org.append(latent_org_in)
            
                
                        
                bs = latent_model_input[0].shape[0]
                t_expand = t.repeat(bs)
                guidance_expand = (torch.tensor(
                    [embedded_guidance_scale] * bs,
                    dtype=torch.float32,
                    device=device,
                ...
                if self.do_classifier_free_guidance:
                    noise_pred_list = []
                    for noise_pred_i in noise_pred:
                        uncond, text = noise_pred_i.chunk(2)
                        noise_pred_list.append(uncond + self.guidance_scale * (text - uncond))

                if self.do_classifier_free_guidance and self.guidance_rescale > 0.0:
                    # Based on 3.4. in https://arxiv.org/pdf/2305.08891.pdf
                    noise_pred = []
                    for pred in noise_pred_list:
                ...
                latents_batched = torch.cat(latents, dim=0) # (4, c, f, h, w)
                noise_pred_batched = torch.cat(noise_pred_list, dim=0)
                
                latents_batched = self.scheduler.step(noise_pred_batched, t, latents_batched, **extra_step_kwargs, return_dict=False)[0]
                
                latents[:] = list(latents_batched.split(1, dim=0))
        ...
        if not output_type == "latent":
            ...
            for latent in latents:
                ...
                if len(latent.shape) == 4:
                    ...
                    raise ValueError(
                        f"Only support latents with shape (b, c, h, w) or (b, c, f, h, w), but got {latent.shape}.")
                ...
                with torch.autocast(device_type="cuda", dtype=vae_dtype, enabled=vae_autocast_enabled):
                    image_i = self.vae.decode(latent, return_dict=False, generator=generator)[0]

                if expand_temporal_dim or image_i.shape[2] == 1:
                    image_i = image_i.squeeze(2)
                image.append(image_i)
        else:
            image = latents

        for i in range(len(image)):
            image[i] = (image[i] / 2 + 0.5).clamp(0, 1)
            # we always cast to float32 as this does not cause significant overhead and is compatible with bfloa16
            image[i] = image[i].cpu().float()
