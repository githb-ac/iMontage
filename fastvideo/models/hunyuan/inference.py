...
class ResizeCropAspectCenter:
    ...
    def _process_frame(self, frame: torch.Tensor) -> torch.Tensor:
        ...
        # 缩放
        frame = F.resize(frame, (new_h, new_w))

        # 居中裁剪
        delta_h = new_h - self.th
        delta_w = new_w - self.tw
        top  = delta_h // 2
        left = delta_w // 2
        frame = F.crop(frame, top, left, self.th, self.tw)
    ...
    def __call__(self, img: torch.Tensor) -> torch.Tensor:
        ...
        # 如果是视频张量 T×C×H×W
            ...
            processed = [self._process_frame(img[t]) for t in range(img.shape[0])]
...
class Inference:
    ...
    def from_pretrained(cls, pretrained_model_path, args, device=None, **kwargs):
        ...
        model = model.to(PRECISION_TO_TYPE[args.precision])
        ...
        fsdp_kwargs, _ = get_dit_fsdp_kwargs(
            model,
            "full",
            master_weight_type=args.precision,
        )
        model = FSDP(
            model,
            **fsdp_kwargs,
...
class HunyuanVideoSampler(Inference):
    ...
    def predict(
        ...
        **kwargs,
        ...
        # ========================================================================
        # Load original video or image
        # ========================================================================
        if media_type == "video":
            assert isinstance(video, str), "Video editing infer should pass original video path."
            ...
            video_org = video_transform(video_org).to(self.device).unsqueeze(0).permute(0, 2, 1, 3, 4) #b, f, c, h, w -> b, c, f, h, w
            
        elif media_type == "image":
            assert isinstance(video, list) and all(isinstance(img, torch.Tensor) for img in video), \
                "Many2one infer should pass a list of image tensors."
            image_transform = transforms.Compose([
                ResizeCropAspectCenter(target_height, target_width),
                transforms.Normalize([0.5], [0.5]),
            ])
            video_org = [image_transform(img).to(self.device).unsqueeze(2) for img in video]
        else:
            raise ValueError(f"Unsupported media_type {media_type}, only 'video' and 'image' are supported.")
        # ========================================================================
        # Print infer args
        # ========================================================================
        debug_str = f"""
