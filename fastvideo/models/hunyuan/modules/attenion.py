...
def parallel_attention(q, k, v, text_mask, image_mask=None, mask_strategy=None):
    ...
    if mask_strategy[0] is not None:
        ...
        # NOTE add image padding mask for multi-out
        if image_mask is not None:
            assert image_mask.shape == (text_mask.shape[0], sequence_length), \
                f"image_mask shape {image_mask.shape} does not match expected {(text_mask.shape[0], sequence_length)}"
