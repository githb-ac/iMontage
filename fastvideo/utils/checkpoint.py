...
from torch.distributed.fsdp import FullOptimStateDictConfig, FullStateDictConfig, ShardedOptimStateDictConfig, ShardedStateDictConfig
...
def save_checkpoint_all(
    ...
    if save_hf:
        # 1) export full model state for HF-style weights
        with FSDP.state_dict_type(
            transformer,
            StateDictType.FULL_STATE_DICT,
            FullStateDictConfig(offload_to_cpu=True, rank0_only=True),
        ):
            cpu_model_state = transformer.state_dict()

        # 2) save HF weights & config (only on rank 0)
        if rank == 0:
            weight_path = os.path.join(save_dir, "diffusion_pytorch_model.safetensors")
            save_file(cpu_model_state, weight_path)
            config_dict = dict(transformer.config)
            config_dict.pop("dtype", None)
            with open(os.path.join(save_dir, "config.json"), "w") as f:
                json.dump(config_dict, f, indent=4)
        main_print(f"--> saved HF weight checkpoint at {save_dir}")

    # 2b) always save scheduler & scaler (only on rank 0)
    if rank == 0:
    ...
    # 3) save sharded model weights & optimizer states
    model_weights_dir = os.path.join(save_dir, "model_weights_state")
    ...
    with FSDP.state_dict_type(
        ...
        ShardedStateDictConfig(offload_to_cpu=True), # NOTE save vram...
        ...
        dist_cp.save(
            ...
            storage_writer=dist_cp.FileSystemWriter(model_weights_dir),
...
def load_checkpoint_all(
    ...
    if os.path.exists(sched_path):
        ...
        main_print(f"warning: scheduler state not found at {sched_path}")
    if scaler is not None:
        scaler_path = os.path.join(checkpoint_dir, "scaler.pt")
        if os.path.exists(scaler_path):
            scaler.load_state_dict(torch.load(scaler_path, map_location="cpu"))
        else:
            main_print(f"warning: scaler state not found at {scaler_path}")

    # 2) 如果 shard 目录存在，就走 sharded 加载；否则 fallback 全量加载
    model_dir = os.path.join(checkpoint_dir, "model_weights_state")
    optim_dir = os.path.join(checkpoint_dir, "model_optimizer_state")

    if os.path.exists(model_dir) and os.path.exists(optim_dir):
        # sharded 模式加载模型和优化器
        with FSDP.state_dict_type(model, StateDictType.SHARDED_STATE_DICT):
            shard_container = {"model": model.state_dict()}
            dist_cp.load(
                state_dict=shard_container,
                storage_reader=dist_cp.FileSystemReader(model_dir),
                planner=DefaultLoadPlanner(),
            )
            model.load_state_dict(shard_container["model"], strict=strict)

            optim_shards = load_sharded_optimizer_state_dict(
                model_state_dict=shard_container["model"],
                optimizer_key="optimizer",
                storage_reader=dist_cp.FileSystemReader(optim_dir),
            )["optimizer"]
            flat_optim = FSDP.optim_state_dict_to_load(
                model=model,
                optim=optimizer,
                optim_state_dict=optim_shards
            )
            optimizer.load_state_dict(flat_optim)

        main_print(f"--> loaded sharded model & optimizer from {model_dir}")

    else:
        # fallback: 全量 safetensors 加载模型，跳过 optimizer
        main_print("shard dirs not found, falling back to full-state safetensors")
        weight_path = os.path.join(checkpoint_dir, "diffusion_pytorch_model.safetensors")
        full_state = load_file(weight_path)
        with FSDP.state_dict_type(
            model,
            StateDictType.FULL_STATE_DICT,
            FullStateDictConfig(offload_to_cpu=True, rank0_only=True),
            ...
            curr.update(full_state)
            model.load_state_dict(curr, strict=strict)
        main_print(f"--> loaded full model state from {weight_path}")
        main_print("optimizer shards skipped in fallback mode")

    # 3) parse step
    step = int(os.path.basename(checkpoint_dir).split("-")[-1])
    return model, optimizer, lr_scheduler, scaler, step